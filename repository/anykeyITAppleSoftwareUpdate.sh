    ...
    if ($AVAILABLEUPDATES=~/(restart)|(shut\sdown)/){
        ...
        if ($LOGGEDINUSER=~/[a-zA-Z]/) {
            ...
            if ($CHANCESLEFT <= 0) {
                ...
                system 'jamf displayMessage -message "Es stehen Updates zur Verfügung welche einen Neustart erfordern. Da der Update-Prozess schon 5 Mal verzögert wurde wird er nur forciert. Bitte sofort alle Dokumente sichern."';
