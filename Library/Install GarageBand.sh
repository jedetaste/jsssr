#!/bin/bash
  ...
  if [ "${localCDN8080}" == "200" ] || [ "${cloudCDN}" != "217.150.247.87" ]; then
    ...
    if [ "${osvers}" -eq 9 ]; then
      ...
      /usr/local/bin/aky garageband10098080
    fi
    ...
    if [ "${osvers}" -eq 9 ]; then
      echo "Upgrading GarageBand for Mavericks"
      /usr/local/bin/aky garageband1009
    fi

    if [ "${osvers}" -eq 10 ]; then
      echo "Upgrading GarageBand for Yosemite"
      /usr/local/bin/aky garageband1010
    fi

    if [ "${osvers}" -eq 11 ]; then
      echo "Upgrading GarageBand for El Capitain"
      /usr/local/bin/aky garageband1011
    fi

    if [ "${osvers}" -eq 12 ]; then
      echo "Upgrading GarageBand for Sierra"
      /usr/local/bin/aky garageband1012
    fi

    if [ "${osvers}" -eq 13 ]; then
      echo "Upgrading GarageBand for High Sierra"
      /usr/local/bin/aky garageband1013
    fi
