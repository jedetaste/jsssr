...
if [[ "$BAILOUT" == "true" ]]; then
  if [[ $(defaults read /Library/LaunchDaemons/$BUNDLE_ID.plist StartInterval 2>/dev/null) -eq 0 ]]; then
    echo "Stopping due to errors, but will try again in $(convert_seconds "$(defaults read /Library/LaunchDaemons/$BUNDLE_ID.plist StartInterval 2>/dev/null)")."
  else
